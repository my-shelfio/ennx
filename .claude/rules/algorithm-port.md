...
- **前提条件（実行可能性）**: 各地域について**目標定員の合計 ≤ 地域上限**。この前提は `FDAInput.__post_init__` で検証する（破ると地域上限超過が起こりうる。#35）
- **手順**（各ラウンド）:
  ...
  ※ 前ラウンドの繰り上げで目標定員を超えてキープ中の者も、ここで一旦待機リストに戻る。この「キープし直し」を省略すると繰り上げ分が地域枠を占有したまま残り、後続の受入で地域上限を超過する（#35 の修正点）
