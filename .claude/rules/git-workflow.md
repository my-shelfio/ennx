...
- ブランチ名は `<prefix>/<イシュー番号>` **のみ**（説明的な文字列を付加しない）。prefix はイシューのラベルから判定する（下表）。例: イシュー #30（`feature` ラベル）→ `feat/30`。
...
- タスクは GitHub のイシュー単位で対応する。なければ先にイシューを登録する（`.github/ISSUE_TEMPLATE/` のテンプレートに準拠し、ラベルを設定する）。マイルストーンは運用しない。既存イシューの確認は `gh issue list` / `gh issue view [番号]` を使う。
...
- **ブランチ名は `<prefix>/<イシュー番号>` の形式のみとし、機能名などの説明的な文字列を付加しない**
  ...
  命名はしない）。複数ラベルが付いている場合や、ラベルからは判断しづらい場合は、
  イシュー本文の内容（新機能か・バグ修正か等）から下表の prefix を判断し、番号のみを
  ブランチ名に用いる。
...
sequenceDiagram
    ...
    loop 要求ごとに繰り返し
        ...
        P1->>C: ③実装
        note over P1,C: ②は任意。本リポジトリには記録先を設けず、<br/>必要な場合はリポジトリ外で記録する
